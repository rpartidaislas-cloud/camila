...
assert.match(compositorSeguro, /post-safety-clip-six-locked-crowns/);
assert.match(compositorSeguro, /geometryLocked:geometriaBloqueada/);
assert.match(compositorSeguro, /mode:'pixel-locked-v4'/);
assert.match(compositorSeguro, /mask:coberturaContrato/);
assert.doesNotMatch(compositorSeguro, /mask:tratamientoActual\.metrics/);
assert.match(html, /contractVersion:'hybrid-2d-v4'/);
...
assert.match(generadorV105, /imageBase64:imagenBloqueadaBase64/);
assert.match(generadorV105, /guideImageBase64:referenciaOriginalBase64/);
assert.match(generadorV105, /guideMimeType:entradaDental\.mimeType\|\|'image\/png'/);
...
assert.match(generadorV105, /IMAGE 1 is already a pixel-registered patient-derived reconstruction/);
assert.match(generadorV105, /IMAGE 2 is the original patient crop/);
...
assert.match(generadorV105, /prepararMascaraCoronasBloqueadasV4/);
assert.match(generadorV105, /Only optical material inside the six exact crown masks may change/);
assert.doesNotMatch(generadorV105, /requiresNewGeneration[\s\S]{0,180}delete S\.pendingGeneratedByView/);
assert.match(generadorV105, /localValidationContract:'hybrid-2d-v4'/);
...
const preparadorMascaraBloqueada=extractFunction('prepararMascaraCoronasBloqueadasV4');
assert.match(preparadorMascaraBloqueada,/renderizarMascaraCoronasSeparadasV4/);
assert.match(preparadorMascaraBloqueada,/dentro\?0:255/);
assert.match(preparadorMascaraBloqueada,/connectedRegion:false,regionPathCount:6,geometryLocked:true/);
assert.match(preparadorMascaraBloqueada,/hybrid-2d-v4-locked-six-crowns/);
assert.doesNotMatch(preparadorMascaraBloqueada,/construirMascaraRegionContinuaHibrida2D/);
const mascaraSeisIslas=extractFunction('renderizarMascaraCoronasSeparadasV4');
assert.match(mascaraSeisIslas,/renderizarMascaraObjetivoDental/);
assert.match(mascaraSeisIslas,/for\(var i=0;i<5;i\+\+\)/);
assert.match(mascaraSeisIslas,/clearRect/);
assert.match(mascaraSeisIslas,/dataset\.regionPathCount='6'/);
...
assert.match(backend, /const isGeometryLockedContract = simulationContract === "hybrid-2d-v4"/);
assert.match(backend, /const isPatientGeometryLock = isGeometryLockedContract && guideLibraryVersion === "patient-geometry-lock-v1"/);
assert.match(backend, /isPhotographicLibraryContract \|\| isGeometryLockedContract/);
...
assert.match(backend, /HYBRID 2D VENEER EDIT V4 — LOCKED PATIENT GEOMETRY/);
assert.match(backend, /exactly six closed crown interiors/);
assert.match(backend, /patient-original-reference\.png/);
...
const evidenciaBloqueadaV4=structuredClone(evidenciaValida);
evidenciaBloqueadaV4.trace.contract='hybrid-2d-v4';
evidenciaBloqueadaV4.mask={sourceCoverage:.71,targetCoverage:1,connectedRegion:false,regionPathCount:6,geometryLocked:true,roiRatio:.065};
const contratoBloqueadoV4=context.evaluarContratoPresentacionV105(evidenciaBloqueadaV4);
assert.equal(contratoBloqueadoV4.accepted,true);
assert.equal(contratoBloqueadoV4.failures.length,0);
const evidenciaBloqueadaSinIslas=structuredClone(evidenciaBloqueadaV4);
evidenciaBloqueadaSinIslas.mask.regionPathCount=1;
assert.equal(context.evaluarContratoPresentacionV105(evidenciaBloqueadaSinIslas).accepted,false);
...
assert.match(guiaAnatomica,/patient-geometry-lock-v1/);
assert.match(guiaAnatomica,/source:'patient-own-six-crowns'/);
assert.match(guiaAnatomica,/registration:'pixel-aligned-locked-crowns'/);
assert.match(guiaAnatomica,/baseImageUrl:diseno\.url/);
assert.match(guiaAnatomica,/targetMaskUrl:diseno\.maskUrl/);
...
console.log('SMYL hybrid-2d-v4: locked patient geometry with six-crown material finishing');
